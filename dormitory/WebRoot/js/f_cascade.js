...
$.fn.cascade=function(opt){//opt即option
	var opts=$.extend({//设置默认参数,参数可选择性覆盖
		url:"address.json",
		firstName:"专业",//一级框前面的文字
		secondName:"班级",
		firstText:"请选择专业",//一级框里默认的选项
		secondText:"请选择班级",
		attrName:"name",
		attrValue:"id"
	},opt||{});
	...
	opts.one=$("<select name='major' id='first_select'><option>"+opts.firstText+"</option></select>");//一级框
	opts.two=$("<select name='classes' id='second_select'><option>"+opts.secondText+"</option></select>");//二级框
